...
export default function ConstructorElementWrapper({ item, index, moveCard }) {
  ...
  return (
    <div ref={ref} style={{ opacity }} onDrop={preventDefault} className={`${styles.innerIngredient} mb-4 mr-2`} data-handler-id={handlerId}>
      <DragIcon />
      <ConstructorElement isLocked={false} text={item.name} price={item.price} thumbnail={item.image} handleClose={delElement}/>
    </div>
  );
}
